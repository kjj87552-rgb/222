...
import { getDirectorStageFocusPoint, getDirectorStageJointPreset, normalizeDirectorStageRotation } from './types'
...
const ACTIVE_CAMERA_COLOR = '#ffd166'
const IDLE_CAMERA_COLOR = '#58a6ff'
...
const createCameraVisual = (
    ...
    const frustum = new THREE.LineSegments(
        ...
        new THREE.LineBasicMaterial({ color, transparent: true, opacity: active ? 0.9 : 0.55 }),
    )
    ...
    if (showNumbers) {
        ...
        if (label) {
            label.position.set(0, 0.48, -0.18)
...
export const DirectorStageViewport = forwardRef<DirectorStageViewportHandle, DirectorStageViewportProps>(function DirectorStageViewport({
    ...
    const syncBackground = useCallback(async () => {
        ...
        try {
            ...
            console.error('[DirectorStageViewport] Failed to load background texture:', error)
            requestRender()
        }
    }, [requestRender])
    ...
    const syncGroundPlan = useCallback(async () => {
        ...
        try {
            ...
            console.error('[DirectorStageViewport] Failed to load ground plan texture:', error)
            requestRender()
        }
    }, [requestRender])
    ...
    useEffect(() => {
        ...
        const grid = new THREE.GridHelper(26, 26, '#4f6bff', '#182040')
        grid.position.y = 0
        scene.add(grid)
