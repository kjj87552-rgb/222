...
import {
  postEditModeAvailable,
  postEditModeDismissed,
  listenForEditModeMessages,
} from './compareEmbed.js';
...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  React.useEffect(() => {
    const cleanup = listenForEditModeMessages((data) => {
  ...
  // Drag handler — pure DOM event wiring, stays inline.
  const onDragStart = (e) => {
    const panel = dragRef.current;
    if (!panel) return;
...
export function TweakNumber({ label, value, min, max, step = 1, unit = '', onChange }) {
  ...
  // Drag/scrub handler — pure DOM event wiring, stays inline.
